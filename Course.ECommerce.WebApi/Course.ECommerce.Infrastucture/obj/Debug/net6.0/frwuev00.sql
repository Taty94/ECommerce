...
CREATE TABLE [DeliveryMethod] (
    ...
    CONSTRAINT [PK_DeliveryMethod] PRIMARY KEY ([Id])
);
GO
...
CREATE TABLE [Order] (
    ...
    [DeliveryMethodId] nvarchar(450) NOT NULL,
    ...
    CONSTRAINT [FK_Order_DeliveryMethod_DeliveryMethodId] FOREIGN KEY ([DeliveryMethodId]) REFERENCES [DeliveryMethod] ([Id]) ON DELETE CASCADE
);
GO
...
CREATE TABLE [ItemOrdered] (
    ...
    [CreationDate] datetime2 NOT NULL,
    [ModifiedDate] datetime2 NULL,
    CONSTRAINT [PK_ItemOrdered] PRIMARY KEY ([Id]),
    CONSTRAINT [FK_ItemOrdered_Order_OrderId] FOREIGN KEY ([OrderId]) REFERENCES [Order] ([Id]) ON DELETE CASCADE
);
GO

CREATE INDEX [IX_ItemOrdered_OrderId] ON [ItemOrdered] ([OrderId]);
GO

CREATE INDEX [IX_Order_DeliveryMethodId] ON [Order] ([DeliveryMethodId]);
GO

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20220717214140_AddOrderEntities-AddTheirConfiguration', N'6.0.6');
GO
...
DROP INDEX [IX_Order_DeliveryMethodId] ON [Order];
DECLARE @var6 sysname;
SELECT @var6 = [d].[name]
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[Order]') AND [c].[name] = N'DeliveryMethodId');
IF @var6 IS NOT NULL EXEC(N'ALTER TABLE [Order] DROP CONSTRAINT [' + @var6 + '];');
ALTER TABLE [Order] ALTER COLUMN [DeliveryMethodId] nvarchar(4) NOT NULL;
CREATE INDEX [IX_Order_DeliveryMethodId] ON [Order] ([DeliveryMethodId]);
GO
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[ItemOrdered]') AND [c].[name] = N'Id');
IF @var7 IS NOT NULL EXEC(N'ALTER TABLE [ItemOrdered] DROP CONSTRAINT [' + @var7 + '];');
ALTER TABLE [ItemOrdered] ALTER COLUMN [Id] nvarchar(4) NOT NULL;
GO
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[DeliveryMethod]') AND [c].[name] = N'Id');
IF @var8 IS NOT NULL EXEC(N'ALTER TABLE [DeliveryMethod] DROP CONSTRAINT [' + @var8 + '];');
ALTER TABLE [DeliveryMethod] ALTER COLUMN [Id] nvarchar(4) NOT NULL;
GO

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20220717215037_ModifiedConfigurationOrder-ItemOrdered-Delivery', N'6.0.6');
GO
